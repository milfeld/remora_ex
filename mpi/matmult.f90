...
! multiply the row with the column 
      ...
      parameter (SIZE_OF_MATRIX = 3600) 
     !parameter (SIZE_OF_MATRIX = 6000) 
     !parameter (SIZE_OF_MATRIX = 7000) 
...
! print out one element of the answer
        write( *,'( "  End, Check: c(",I5,",",I5," ) = ",E17.10)') matsize,matsize,c(matsize,matsize)
      else 
...
! multiply the matrices here using C(i,j) += sum (A(i,k)* B(k,j))
            ...
            if(mod_count==0) write(6,'("-")', ADVANCE='NO')
            if(mod_count==0) call flush(6)
          endif 
        end do
      if(myid==1) write(*,'(" ")', ADVANCE='YES')
      endif
